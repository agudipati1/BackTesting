 ...
 *
 */
public class PriceMovingAverageHelper {
	private int length = 0;
	private boolean simpleMA = true;

	public PriceMovingAverageHelper(int length, boolean simpleMA) {
		this.length = length;
	...
	public BigDecimal processTick(final InstrumentPriceModel aTick, final BigDecimal lastMA) {			
		...
		if (aTick.getDateType() == TradeDateType.TRADING_DATE) {
			if(simpleMA || rollingTicks.size() <= length) {
				...
				rollingSum = rollingSum.add(aTick.getClose());
	
				if(rollingTicks.size() > length) {
					final InstrumentPriceModel lastTick = rollingTicks.remove(0);
					rollingSum = rollingSum.subtract(lastTick.getClose());
				...
				if(lastMA != null) {
					...
					avg = (((aTick.getClose().subtract(lastMA)).multiply(multiplier))).add(lastMA);
				}
			}
		} else {
